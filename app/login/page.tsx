...
export default function LoginPage() {
  ...
  return (
    <div className="w-full bg-gray-50 min-h-screen flex items-center justify-center py-12 px-4">
      <div className="bg-white rounded-xl shadow-2xl border border-gray-100 p-8 w-full max-w-md flex flex-col items-center">
        <h1 className="text-2xl md:text-3xl font-bold mb-2 text-balance text-primary text-center">Login</h1>
        <p className="mb-8 text-muted-foreground text-center text-base">Sign in to your account</p>
        <form onSubmit={handleSubmit} className="space-y-6 w-full">
          ...
          {success && <div className="bg-green-100 text-green-700 p-2 rounded mb-2 text-sm text-center border border-green-200">Login successful!</div>}
          <button type="submit" disabled={loading} className="bg-primary text-primary-foreground px-8 py-3 rounded-md hover:bg-primary/90 focus:ring-2 focus:ring-primary/40 transition text-lg font-semibold w-full disabled:opacity-60">{loading ? 'Logging in...' : 'Login'}</button>
        </form>
